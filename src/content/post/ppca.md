...
description: "Deriving probabilistic PCA as the maximum-likelihood solution of a linear-Gaussian latent-variable model, and how it reduces to classical PCA."
...
Given $N$ i.i.d. datapoints $\lbrace t_i\rbrace_{i=1}^N$, 

$$
\begin{align*}
p(T | W,\psi, \mu) &= \prod_{i=1}^N p(t_i | W,\psi, \mu)\\
&= (2\pi)^{-ND/2}|C|^{-N/2}\exp{\Big\lbrace -\frac{1}{2}\sum_{i=1}^N (t_i-\mu)^TC^{-1}(t_i-\mu) \Big\rbrace}
\end{align*}
$$
...
![PPCA output](./ppca_output.png)

*We start from the observed data (in blue) and through the PPCA max log likelihood we find the optimized parameters, which once plugged in the linear relation and sample some values from the latent space we effectively generate a new dataset (in red) with similar distribution as the observed dataset.*
